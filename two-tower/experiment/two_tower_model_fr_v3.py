...
class TwoTowerModelFRV3:
    def __init__(self, train_file_dir, test_file_dir, item_tower_file, is_train, item_embedding_size,
        ...
        self.label, self.user_id, self.user_type, self.item_id, self.item_type, self.class_id, self.tag_ids, self.author_id, self.action_time, self.act_hour, \
        self.act_isweekend, self.act_noon, self.act_day, self.dev_type, self.dev_brand, self.dev_brand_type, self.dev_brand_level, self.dev_os, self.dev_net, \
        self.client_type, self.dev_carrier, self.dev_lang, self.dev_loc, self.dev_ip_short, self.location_id, self.user_city, self.user_city_level, self.class_sab, \
        self.item_age, self.click_seq_50size, self.gender, self.age, self.consume_level, self.career, self.education, self.user_app_list, self.user_set_cates, \
        self.unclick_seq_50size, self.ic1_ctr, self.ic3_ctr, self.ic7_ctr, self.ic14_ctr, self.ic30_ctr, self.ic60_ctr, self.uc1_ct, self.uc3_ctr, self.uc7_ctr, self.uc14_ctr, \
        self.uc30_ctr = self.train_iterator.get_next()
    ...
    def decode_train_line(self, line):
        defaults = [['0']] * 49
        label, user_id, user_type, item_id, item_type, class_id, tag_ids, author_id, action_time, act_hour, \
        act_isweekend, act_noon, act_day, dev_type, dev_brand, dev_brand_type, dev_brand_level, dev_os, dev_net, \
        client_type, dev_carrier, dev_lang, dev_loc, dev_ip_short, location_id, user_city, user_city_level, class_sab, \
        item_age, click_seq_50size, gender, age, consume_level, career, education, user_app_list, user_set_cates, \
        unclick_seq_50size, ic1_ctr, ic3_ctr, ic7_ctr, ic14_ctr, ic30_ctr, ic60_ctr, uc1_ct, uc3_ctr, uc7_ctr, uc14_ctr, \
        uc30_ctr = tf.decode_csv(line, defaults)
        ...
        # user_id = tf.cast(user_id, dtype=tf.int64)
        # gender = tf.cast(gender, dtype=tf.int64)
        return label, user_id, user_type, item_id, item_type, class_id, tag_ids, author_id, action_time, act_hour, \
        act_isweekend, act_noon, act_day, dev_type, dev_brand, dev_brand_type, dev_brand_level, dev_os, dev_net, \
        client_type, dev_carrier, dev_lang, dev_loc, dev_ip_short, location_id, user_city, user_city_level, class_sab, \
        item_age, click_seq_50size, gender, age, consume_level, career, education, user_app_list, user_set_cates, \
        unclick_seq_50size, ic1_ctr, ic3_ctr, ic7_ctr, ic14_ctr, ic30_ctr, ic60_ctr, uc1_ct, uc3_ctr, uc7_ctr, uc14_ctr, \
    ...
    def build_user_train_data(self, train_data_file):
        ...
        dataset = dataset.batch(batch_size=self.batch_size).repeat(self.train_epoches)
        return dataset
