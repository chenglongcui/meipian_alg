...
class TwoTowerModelFRV6:
    def __init__(self,
        ...
        if self.mode == "train":
            ...
            # dataset
            self.dataset = self.build_user_train_data(self.tables)
    ...
    def decode_train_line_online(self, *args):
        # defaults = [[0]] + [['0']] + [[0]] * 2 + [['0']] * 3
        user_id, user_click_item_list, gender, client_type, target_item_list, target_cate_list, target_tag_list = args
    ...
    def build_user_train_data(self, train_data_file):
        ...
        if self.local:
            ...
            dataset = tf.data.TableRecordDataset(train_data_file, record_defaults=self.record_defaults, slice_count=1,
                                                 slice_id=0)
            dataset = dataset.batch(self.batch_size).repeat(self.train_epoches)
            dataset = dataset.map(self.decode_train_line_online)
    ...
    def build_sequence_features(self):
        ...
        # self.click_seq_50size_array = tf.reshape(tf.strings.split(self.user_click_item_list, sep=";").values,
        #                                          shape=[tf.shape(self.user_id)[0], -1])
        self.user_click_item_list_shape = tf.shape(self.user_click_item_list)
        self.click_seq_50size_array = tf.strings.split(self.user_click_item_list, sep=";").values
