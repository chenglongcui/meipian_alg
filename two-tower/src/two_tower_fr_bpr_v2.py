...
class TwoTowerModelFRBPRV2:
    def __init__(self, train_file_dir, test_file_dir, item_tower_file, is_train, item_embedding_size,
                 cate_embedding_size, tag_embedding_size, batch_size, learning_rate, output_table, local, top_k_num,
                 neg_sample_num, sess):
    ...
    def get_seq_avg_embedding(self, item_embedding, user_click_item_list, item_id_list_len_batch, embedding_size):
        ...
        self.embedding_mask = tf.sequence_mask(item_id_list_len_batch, tf.shape(user_click_item_list)[1],
                                               dtype=tf.float32)
        embedding_mask = tf.expand_dims(self.embedding_mask, -1)
        embedding_mask = tf.tile(embedding_mask, [1, 1, embedding_size])
        embedding_mask_2 = self.embed_init * embedding_mask
    ...
    def build_model(self):
        ...
        with tf.name_scope("user_tower"):
            with tf.name_scope('user_embedding'):
                ...
                self.user_embed = tf.concat(
                    [self.user_item_click_avg_embed, self.gender_one_hot], axis=-1)
        ...
        with tf.name_scope("item_tower"):
            ...
            self.target_item_id_embed = tf.nn.embedding_lookup(self.item_embedding, self.target_item_idx)
            # self.item_id_bias = tf.nn.embedding_lookup(self.item_embedding_b, self.target_tag_idx)
            self.target_cate_id_embed = tf.nn.embedding_lookup(self.cate_embedding, self.target_cate_idx)
            self.target_tag_id_embed = tf.nn.embedding_lookup(self.tag_embedding, self.target_tag_idx)
            self.target_embed = tf.concat(
                [self.target_item_id_embed, self.target_cate_id_embed, self.target_tag_id_embed], axis=-1)

            with tf.name_scope('item_layers'):
                bn = tf.layers.batch_normalization(inputs=self.target_embed, name='item_bn', )
                item_layer_1 = tf.layers.dense(bn, self.item_embedding_size, activation=tf.nn.tanh, name='item_first',
                                               kernel_initializer=tf.glorot_normal_initializer())
            self.item_embed_split = tf.split(item_layer_1, 2, 1)
    ...
    def train_model(self):
        ...
        gradients = tf.gradients(loss_merge, self.trainable_params)
        clip_gradients, _ = tf.clip_by_global_norm(gradients, 5)
        train_op = optimizer.apply_gradients(
            zip(clip_gradients, self.trainable_params), global_step=self.global_step)
        return train_op, loss_merge, bpr_losses, auc
