...
class TTFRCTRModelV2:
    def __init__(self,
        ...
        if self.mode == "train":
            # build dataset
            self.features, self.labels = self.build_user_train_data(self.train_file_dir)
    ...
    def build_user_train_data(self, train_data_file):
        def parse_csv(value):
            columns = tf.decode_csv(value, field_delim=';', record_defaults=self._CSV_COLUMN_DEFAULTS)
        ...
        dataset = dataset.repeat(self.train_epoches)
    ...
    def build_sequence_features(self):
        self.click_seq_50size_idx = tf.string_to_hash_bucket_fast(tf.as_string(self.click_seq_50size_ex),
                                                                  self.ITEM_CNT)
