...
class TTFRCTRModelV5:
    def __init__(self,
        ...
        if self.mode in ["train", "eval"]:
            ...
            self.dev_os, self.dev_net, self.client_type, self.dev_carrier, self.click_seq_50size, self.gender, self.age, \
            self.consume_level, self.unclick_seq_50size, self.feed_type, self.collection_id = self.build_train_batch_data(
                self.train_file_dir, self.batch_size, self.train_epoches)

        # self.ic1_ctr = tf.string_to_number(self.ic1_ctr, tf.float32)
        # self.ic3_ctr = tf.string_to_number(self.ic3_ctr, tf.float32)
        # self.ic7_ctr = tf.string_to_number(self.ic7_ctr, tf.float32)
        # self.ic14_ctr = tf.string_to_number(self.ic14_ctr, tf.float32)
        # self.ic30_ctr = tf.string_to_number(self.ic30_ctr, tf.float32)
        # self.ic60_ctr = tf.string_to_number(self.ic60_ctr, tf.float32)
        #
        # self.uc1_ctr = tf.string_to_number(self.uc1_ctr, tf.float32)
        # self.uc3_ctr = tf.string_to_number(self.uc3_ctr, tf.float32)
        # self.uc7_ctr = tf.string_to_number(self.uc7_ctr, tf.float32)
        # self.uc14_ctr = tf.string_to_number(self.uc14_ctr, tf.float32)
        # self.uc30_ctr = tf.string_to_number(self.uc30_ctr, tf.float32)
        ...
        tensor_info_feed_type = tf.saved_model.utils.build_tensor_info(self.feed_type)
        tensor_info_collection_id = tf.saved_model.utils.build_tensor_info(self.collection_id)
        # tensor_info_ic1_ctr = tf.saved_model.utils.build_tensor_info(self.ic1_ctr)
        # tensor_info_ic3_ctr = tf.saved_model.utils.build_tensor_info(self.ic3_ctr)
        # tensor_info_ic7_ctr = tf.saved_model.utils.build_tensor_info(self.ic7_ctr)
        # tensor_info_ic14_ctr = tf.saved_model.utils.build_tensor_info(self.ic14_ctr)
        # tensor_info_ic30_ctr = tf.saved_model.utils.build_tensor_info(self.ic30_ctr)
        # tensor_info_ic60_ctr = tf.saved_model.utils.build_tensor_info(self.ic60_ctr)
        # tensor_info_uc1_ctr = tf.saved_model.utils.build_tensor_info(self.uc1_ctr)
        # tensor_info_uc3_ctr = tf.saved_model.utils.build_tensor_info(self.uc3_ctr)
        # tensor_info_uc7_ctr = tf.saved_model.utils.build_tensor_info(self.uc7_ctr)
        # tensor_info_uc14_ctr = tf.saved_model.utils.build_tensor_info(self.uc14_ctr)
        # tensor_info_uc30_ctr = tf.saved_model.utils.build_tensor_info(self.uc30_ctr)
        ...
        self.saved_model_inputs = {
            ...
            "feed_type": tensor_info_feed_type,
            "collection_id": tensor_info_collection_id,
            # "ic1_ctr": tensor_info_ic1_ctr,
            # "ic3_ctr": tensor_info_ic3_ctr,
            # "ic7_ctr": tensor_info_ic7_ctr,
            # "ic14_ctr": tensor_info_ic14_ctr,
            # "ic30_ctr": tensor_info_ic30_ctr,
            # "ic60_ctr": tensor_info_ic60_ctr,
            # "uc1_ctr": tensor_info_uc1_ctr,
            # "uc3_ctr": tensor_info_uc3_ctr,
            # "uc7_ctr": tensor_info_uc7_ctr,
            # "uc14_ctr": tensor_info_uc14_ctr,
            # "uc30_ctr": tensor_info_uc30_ctr,
        }
    ...
    def read_train_data(self, file_queue):
        ...
        dev_os, dev_net, client_type, dev_carrier, click_seq_50size, gender, age, consume_level, unclick_seq_50size, feed_type, \
        collection_id = tf.decode_csv(value, defaults)
        ...
        return click_label, user_id, item_id, class_id, tag_ids, dev_type, dev_brand, dev_brand_type, dev_os, dev_net, \
               client_type, dev_carrier, click_seq_50size, gender, age, consume_level, unclick_seq_50size, feed_type, \
               collection_id
    ...
    def build_train_batch_data(self, train_data_file, batch_size, num_epochs):
        ...
        click_label, user_id, item_id, class_id, tag_ids, dev_type, dev_brand, dev_brand_type, dev_os, dev_net, \
        client_type, dev_carrier, click_seq_50size, gender, age, consume_level, unclick_seq_50size, feed_type, \
        collection_id = self.read_train_data(
            file_queue)
        ...
        click_seq_50size_batch, gender_batch, age_batch, consume_level_batch, unclick_seq_50size_batch, feed_type_batch, \
        collection_id_batch = tf.train.batch(
            [click_label, user_id, item_id, class_id, tag_ids, dev_type, dev_brand, dev_brand_type,
             dev_os, dev_net, client_type, dev_carrier, click_seq_50size, gender, age, consume_level,
             unclick_seq_50size, feed_type, collection_id],
        ...
        return click_label_batch, user_id_batch, item_id_batch, class_id_batch, tag_ids_batch, dev_type_batch, dev_brand_batch, \
               ...
               click_seq_50size_batch, gender_batch, age_batch, consume_level_batch, unclick_seq_50size_batch, feed_type_batch, \
               collection_id_batch
        # ic1_ctr_batch, ic3_ct_batch, ic7_ctr_batch, ic14_ctr_batch, ic30_ctr_batch, ic60_ctr_batch, \
        # uc1_ctr_batch, uc3_ctr_batch, uc7_ctr_batch, uc14_ctr_batch, uc30_ctr_batch
    ...
    def build_categorial_features(self):
        self.class_id_idx = tf.string_to_hash_bucket_fast(self.class_id, self.CATE_CNT)
        self.class_id_one_hot = tf.one_hot(self.class_id_idx, self.CATE_CNT)

        self.collection_id_idx = tf.string_to_hash_bucket_fast(self.collection_id, self.COLLECTION_CNT)
        self.collection_id_one_hot = tf.one_hot(self.collection_id_idx, self.COLLECTION_CNT)
        ...
        self.feed_type_idx = tf.string_to_hash_bucket_fast(self.feed_type, self.FEED_TYPE_CNT)
        self.feed_type_one_hot = tf.one_hot(self.feed_type_idx, self.FEED_TYPE_CNT)

    def build_numerical_features(self):
        self.ic1_ctr = tf.expand_dims(self.ic1_ctr, 1)
        self.ic3_ctr = tf.expand_dims(self.ic3_ctr, 1)
        self.ic7_ctr = tf.expand_dims(self.ic7_ctr, 1)
        self.ic14_ctr = tf.expand_dims(self.ic14_ctr, 1)
        self.ic30_ctr = tf.expand_dims(self.ic30_ctr, 1)
        self.ic60_ctr = tf.expand_dims(self.ic60_ctr, 1)

        self.uc1_ctr = tf.expand_dims(self.uc1_ctr, 1)
        self.uc3_ctr = tf.expand_dims(self.uc3_ctr, 1)
        self.uc7_ctr = tf.expand_dims(self.uc7_ctr, 1)
        self.uc14_ctr = tf.expand_dims(self.uc14_ctr, 1)
        self.uc30_ctr = tf.expand_dims(self.uc30_ctr, 1)
    ...
    def build_model(self):
        ...
        with tf.name_scope("user_tower"):
            ...
            self.user_embed_concat = tf.concat(
                [self.click_seq_50size_embed, self.uncclick_seq_50size_embed, self.tag_ids_embed,
                 ...
                 self.dev_net_one_hot, self.dev_brand_one_hot, self.dev_os_one_hot, self.feed_type_one_hot],
                axis=-1)
        ...
        with tf.name_scope("item_tower"):
            item_embed_concat = tf.concat(
                [self.item_id_embed, self.tag_ids_embed, self.class_id_one_hot, self.collection_id_one_hot], axis=-1)
