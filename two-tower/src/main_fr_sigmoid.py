...
from two_tower_model_fr_sigmoid import TwoTowerModelFRSIGMOID
...
def main(_):
    ...
    with tf.Session() as sess:
        ...
        two_tower_model = TwoTowerModelFRSIGMOID(train_file_dir=train_file_name,
        ...
        try:
            ...
            if is_train == 1:
                train, losses, aucs = two_tower_model.train_model()
            ...
            while True:
                ...
                if is_train:
                    ...
                    #
                    _, loss, learning_rate, auc = sess.run(
                        [train, losses, aucs, two_tower_model.learning_rate])
                    loss_sum += loss
                    # bpr_sum += bpr_loss
                    # auc_sum += auc
                    if train_step % two_tower_model.PRINT_STEP == 0:
                        if train_step == 0:
                            print(
                                'time: %s\tEpoch: %d\tGlobal_Train_Step: %d\tTrain_loss: %.8f\tauc:%.8f\tLearning_rate:%.8f'
                                % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()),
                                   two_tower_model.train_epoches,
                                   train_step, loss_sum, auc, learning_rate))
