...
class TwoTowerModelFRV1:
    def __init__(self, train_file_dir, pred_file_dir, item_tower_file, is_train, item_embedding_size,
        ...
        if self.is_train:
            ...
            self.user_id, self.user_click_item_list, self.gender, self.target_item_list, self.target_cate_list, self.target_tag_list, _ = self.iterator.get_next()
    ...
    def decode_train_line(self, line):
        defaults = [[0]] + [['0']] + [[0]] + [['0']] * 4
        user_id, user_click_item_list, gender, target_item_list, target_cate_list, target_tag_list, train_label = tf.decode_csv(
            line, defaults)
        ...
        train_label = tf.string_to_number(tf.string_split([train_label], ';').values, tf.int64)

        user_id = tf.cast(user_id, dtype=tf.int64)
        gender = tf.cast(gender, dtype=tf.int64)
        # target_item = tf.cast(target_item, dtype=tf.int64)
        # target_cate = tf.cast(target_cate, dtype=tf.int64)
        # target_tag = tf.cast(target_tag, dtype=tf.int64)
        return user_id, user_click_item_list, gender, target_item_list, target_cate_list, target_tag_list, train_label
    ...
    def build_user_train_data(self, train_data_file):
        ...
        dataset = dataset.padded_batch(batch_size=self.batch_size, padded_shapes=(
            [], [None], [], [None], [None], [None], [None])).repeat(self.train_epoches).shuffle(buffer_size=2)
        return dataset
    ...
    def build_user_test_data(self, pred_data_file):
        ...
        dataset = dataset.padded_batch(batch_size=self.batch_size, padded_shapes=([], [None], [])).repeat(
            self.pred_epoches)
        return dataset
    ...
    def build_model(self):
        ...
        # self.user_click_item_list_idx_hash = tf.string_to_hash_bucket_fast(tf.castself.user_click_item_list, self.ITEM_MOD)
        #
        ...
        with tf.name_scope("user_tower"):
            ...
            with tf.name_scope('layers'):
                ...
                layer_1 = tf.layers.dense(bn, 512, activation=tf.nn.selu, name='first',
                                          kernel_initializer=self.selu_initializer)
                layer_2 = tf.layers.dense(layer_1, 256, activation=tf.nn.selu, name='second',
                                          kernel_initializer=self.selu_initializer)
                layer_3 = tf.layers.dense(layer_2, self.item_embedding_size, activation=tf.nn.selu,
                                          name='user_final', kernel_initializer=self.selu_initializer)
        ...
        with tf.name_scope("item_tower"):
            ...
            if self.is_train:
                self.target_item_idx = tf.mod(self.target_item_list, self.ITEM_MOD)
                self.target_cate_idx = tf.mod(self.target_cate_list, self.CATE_MOD)
                self.target_tag_idx = tf.mod(self.target_tag_list, self.TAG_MOD)
            else:
                self.target_item_idx = tf.mod(self.item_vobabulary, self.ITEM_MOD)
                self.target_cate_idx = tf.mod(self.item_cate_mapping, self.CATE_MOD)
                self.target_tag_idx = tf.mod(self.item_tag_mapping, self.TAG_MOD)
            ...
            dense_layer_1 = tf.layers.dense(bn, 128, activation=tf.nn.selu,
                                            name='first_dense', kernel_initializer=self.selu_initializer)
            dense_layer_2 = tf.layers.dense(dense_layer_1, 64, activation=tf.nn.selu, name='second_dense',
                                            kernel_initializer=self.selu_initializer)
            dense_layer_3 = tf.layers.dense(dense_layer_2, self.item_embedding_size, activation=tf.nn.selu,
                                            name='item_embed_final', kernel_initializer=self.selu_initializer)

            self.item_embeding_final = dense_layer_3
            if self.is_train:
                ...
                self.logits = tf.matmul(self.user_embedding_final, self.item_embeding_final, transpose_b=True)
