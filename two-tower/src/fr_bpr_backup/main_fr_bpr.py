...
from two_tower_model_fr_bpr import TwoTowerModelFRBPR
...
def main(_):
    ...
    with tf.Session() as sess:
        ...
        two_tower_model = TwoTowerModelFRBPR(train_file_dir=train_file_name,
        ...
        try:
            ...
            if is_train == 1:
                train, losses, bpr_losses, aucs, lr = two_tower_model.train_model()
                ...
                aucs = two_tower_model.evaluate_auc()
                # if local == 0:
            ...
            while True:
                ...
                if is_train:
                    ...
                    _, loss, bpr_loss, auc, learning_rate = sess.run(
                        [train, losses, bpr_losses, aucs, lr])
                    loss_sum += loss
                    bpr_sum += bpr_loss
                    auc_sum += auc
                    if train_step % two_tower_model.PRINT_STEP == 0:
                        if train_step == 0:
                            print(
                                'time: %s\tEpoch: %d\tGlobal_Train_Step: %d\tTrain_loss: %.8f\tbpr_loss: %.8f\tmeas_auc: %.6f\tLearning_rate:%.8f'
                                % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()),
                                   two_tower_model.train_epoches,
                                   train_step, loss_sum, bpr_sum, auc, learning_rate))

                        else:
                            print(
                                'time: %s\tEpoch: %d\tGlobal_Train_Step: %d\tTrain_loss: %.8f\tbpr_loss: %.8f\tmean_auc: %.6f\tLearning_rate:%.8f'
                                % (
                                    time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()),
                                    two_tower_model.train_epoches,
                                    train_step,
                                    loss_sum / two_tower_model.PRINT_STEP,
                                    bpr_sum / two_tower_model.PRINT_STEP,
                    ...
                    # local test
                    # user_ids, user_embedding_finals,avg = sess.run(
                    #     [
                    #         two_tower_model.user_click_item_list,
                    #         two_tower_model.embed_init,
                    #         two_tower_model.user_item_click_avg_embed,
                    #         # two_tower_model.item_embed_split,
                    #     ])
                    # print("user_ids")
                    # print(user_ids)
                    # print("user_embedding_finals")
                    # print(user_embedding_finals)
                    # print("user final")
                    # print(avg)
                    # print("multiple")
                    ...
                    if pred_step % two_tower_model.PRINT_STEP == 0:
                        print('time: %s\tstep: %d\tmeas_auc: %.6f\t' % (
                            time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), pred_step, auc_sum / pred_step))
                    # user_ids, user_embedding_finals = sess.run(
                    #     [
                    #         two_tower_model.user_id,
                    #         two_tower_model.logits,
                    #         # two_tower_model.neg_embed_final,
                    #         # two_tower_model.item_embed_split,
                    #     ])
                    # print("user_ids")
                    # print(user_ids)
