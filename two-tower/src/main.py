...
from two_tower_model_fr import TwoTowerModelFR
...
def main(_):
    ...
    with tf.Session() as sess:
        ...
        two_tower_model = TwoTowerModelFR(train_file_dir=train_file_name,
            ...
            while True:
                ...
                if is_train:
                    ...
                    if train_step % two_tower_model.PRINT_STEP == 0:
                        if train_step == 0:
                            print('time: %s\tEpoch: %d\tGlobal_Train_Step: %d\tTrain_loss: %.8f\tLearning_rate:%.8f'
                                  % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), two_tower_model.epoches,
                    ...
                    if local == 0:
                        ...
                        user_embeddings, item_embeddings = sess.run(
                            [two_tower_model.user_embedding_final, two_tower_model.item_embed_final])
                        print("users:")
