...
class TwoTowerModelFRV2:
    def __init__(self, train_file_dir,
        ...
        if self.is_train:
            ...
            self.user_id, self.user_click_item_list, self.gender, self.target_item_list, self.target_cate_list, self.target_tag_list = self.iterator.get_next()
            ...
            self.user_id, self.user_click_item_list, self.gender, self.true_click = self.iterator.get_next()
            self.training_init_op = self.iterator.make_initializer(self.dataset)
    ...
    def build_model(self):
        ...
        with tf.name_scope("user_tower"):
            ...
            with tf.name_scope('layers'):
                ...
                user_layer_3 = tf.layers.dense(user_layer_2, 64, activation=tf.nn.relu, name='user_third',
                                               kernel_initializer=tf.glorot_normal_initializer())
                user_layer_4 = tf.layers.dense(user_layer_3,
                                               self.item_embedding_size + self.cate_embedding_size + self.tag_embedding_size,
                                               activation=tf.nn.relu,
                                               name='user_final', kernel_initializer=tf.glorot_normal_initializer())
        ...
        with tf.name_scope("item_tower"):
            ...
            #                                name='item_first', kernel_initializer=tf.glorot_normal_initializer())
            ...
            #                                name='item_third', kernel_initializer=tf.glorot_normal_initializer())
            # item_layer_4 = tf.layers.dense(item_layer_3, self.item_embedding_size, activation=tf.nn.relu,
            #                                name='item_final', kernel_initializer=tf.glorot_normal_initializer())

            # self.item_embeding_final = item_layer_4
            self.item_embeding_final = self.item_embed_merge
            if self.is_train:
