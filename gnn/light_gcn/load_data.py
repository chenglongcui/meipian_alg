...
class Data(object):
    def __init__(self, path, batch_size):
        ...
        # 构建user-item的稀疏矩阵，其中user为行，item为列，
        # dok_matrix为系数矩阵存储结构，继承自dict，key是(row,col)构成的二元组，value是非0元素。
        ...
        with tf.gfile.Open(train_file, 'r') as f_train:
            with tf.gfile.Open(test_file, 'r') as f_test:
    ...
    # 构建邻接矩阵（用户数+item数）*(用户数*item数)的方阵,此处可参考论文3.1.3小节
